    ...
    node {
        ...
        withEnv(["PATH+MAVEN=${tool 'm3'}/bin"]) {
            if(FULL_BUILD) {
                ...
                stash name: "artifact", includes: "target/app-1.0-*.war"
            }
        }
    }
}
...
stage('Deploy') {
    node {
        ...
        if(!FULL_BUILD) { //takes the last version from repo
            sh "curl -o metadata.xml -s http://${NEXUS_URL}/repository/ansible/${repoPath}/maven-metadata.xml"
            version = sh script: 'xmllint metadata.xml --xpath "string(//latest)"',
                         returnStdout: true
        }
        def artifactUrl = "http://${NEXUS_URL}/repository/ansible/${repoPath}/${version}/${pom.artifactId}-${version}.war"
